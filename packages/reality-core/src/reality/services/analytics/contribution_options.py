"""Bounded discovery of retained joint contribution confirmations, not their values."""
...
def contribution_review_options(
    ...
    if type(limit) is not int or not 1 <= limit <= 50:
        raise core.InvalidOperation(
            "Contribution selection limit must be between 1 and 50."
        )
    if cursor is not None and (
        type(cursor) is not str or not cursor.strip() or len(cursor) > 128
    ):
        raise core.InvalidOperation("Invalid contribution selection cursor.")
    ...
    member, inventory = CostInventoryMember, CostInventoryReview
    options = (
        select(
            review.action_id.label("action_id"),
            func.min(inventory.effective_at).label("effective_at"),
            ...
            func.max(review.event_sequence).label("sequence"),
        )
        .select_from(review)
        .join(
        ...
        .join(
            member,
            (member.tenant_id == tenant_id) & (member.id == review.inventory_member_id),
        )
        .join(
            inventory,
            (inventory.tenant_id == tenant_id) & (inventory.id == member.review_id),
        )
        .join(
            policy, (policy.tenant_id == tenant_id) & (policy.id == inventory.policy_id)
        )
        ...
        .having(
            ...
            func.count(func.distinct(CostRevenueMatchBasis.document_line_id))
            == func.count(),
            func.count(func.distinct(inventory.action_id)) == 1,
            func.count(func.distinct(inventory.effective_at)) == 1,
            func.count(func.distinct(review.knowledge_at)) == 1,
            func.count(func.distinct(review.event_sequence)) == 1,
    ...
    with session.no_autoflush:
        ...
        if cursor is not None:
            ...
            if sequence is None:
                raise core.NotFound("Contribution selection not found.")
