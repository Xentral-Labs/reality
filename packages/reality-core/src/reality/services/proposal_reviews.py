...
ReviewKind = Literal["import", "reference", "analytics_report", "delivery", "common", "retired"]

REFERENCE_TOOLS = frozenset(
    {"party_create", "party_update", "item_create", "item_update", "location_create", "location_update"}
)
...
def proposal_routing(proposal: ChangeProposal) -> dict[str, str]:
    ...
    kind: ReviewKind = classify_proposal(tool, arguments) if arguments is not None else "retired"
...
def proposal_review(session: Session, tenant_id: str, proposal_id: str) -> dict[str, Any]:
