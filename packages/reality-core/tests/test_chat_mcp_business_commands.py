...
def test_warehouse_identity_and_payment_term_use_proposal_boundary(session, business):
    ...
    assert (
        record_by_id(session, HandlingUnit, handling["records"][0]["id"]).nve
        == "340123450000000001"
    )
    assert record_by_id(session, PaymentTerm, term["records"][0]["id"]).due_days == 14
...
def test_source_configuration_uses_existing_service(session, business):
    ...
    source_system = record_by_id(session, SourceSystem, output["records"][0]["id"])
