...
from reality.services.analytics import contribution_options
...
def test_options_are_scoped_bounded_and_not_cache_readiness(
    ...
    try:
        first = run_read_tool(
            session, tenant, "graph.contribution_reviews.list", {"limit": 1}
        )
    finally:
    ...
    assert first["items"][0]["position_count"] == 2
    ...
    second = contribution_options.contribution_review_options(
...
def test_foreign_and_missing_cursors_do_not_disclose(session, business, cost_owner):
    ...
    assert (
        contribution_options.contribution_review_options(session, foreign.id)["items"]
        == []
    )
    for tenant, cursor in [(foreign.id, action), (foreign.id, "missing")]:
        with pytest.raises(core.NotFound, match="Contribution selection not found"):
            contribution_options.contribution_review_options(
                session, tenant, cursor=cursor
            )
...
def test_http_metadata_and_foreign_cursor(session, business, cost_owner):
    ...
    with TestClient(app) as client:
        path = f"/tenants/{business.tenant.id}/analytics/graph/contribution-reviews"
        response = client.get(path)
        assert response.status_code == 200, response.text
        assert response.json()["items"][0]["action_id"] == action
        hidden = client.get(
            f"/tenants/{foreign.id}/analytics/graph/contribution-reviews",
