...
def test_item_preview_points_its_location_rows_at_the_pair(session, places):
    sections = operational_preview(
        session, places["tenant"], "item", places["item"].id
    )
...
def test_location_stock_rows_point_at_the_pair_and_carry_the_unit(session, places):
    ...
    row = next(entry for entry in stock["rows"] if entry["label"] == places["item"].name)
    assert row["link"] == {"kind": "stock", "id": pair(places)}
...
def test_scoped_stock_lists_only_items_with_records_there(session, places):
    ...
    assert quiet.id in {row["id"] for row in register(session, places, "stock")["items"]}
...
def test_scoped_reservations_hold_that_location_only(session, places, business):
    ...
    rotterdam = register(session, places, "reservations", location_id=places["rotterdam"].id)
...
def test_scoped_movements_match_either_side(session, places):
    rotterdam = register(session, places, "movements", location_id=places["rotterdam"].id)
    singapore = register(session, places, "movements", location_id=places["singapore"].id)
