...
Assessed against `main` at 4dc658f9 (2026-09-26) by reading tests, services and specs; no suite
was run for this inventory. Evidence paths are relative to `packages/reality-core/` unless they
start with `packages/`, `specs/` or `docs/`. Re-measure a row before building on it.

## Summary

228 scenarios: 51 covered, 82 partial, 19 missing, 73 gap, 3 out.

| Section | covered | partial | missing | gap | out |
|---|---|---|---|---|---|
| A Order intake and changes | 5 | 9 | 2 | 8 | |
| B Availability and reservation | 4 | 7 | 1 | 6 | |
| C Payment and release | 8 | 5 | 1 | 4 | |
| D Shipment, split and merge | 2 | 4 | 2 | 11 | |
| E Customer invoice and credit | 5 | 5 | 2 | | |
| F Returns and complaints | 4 | 6 | 2 | 1 | |
| G Purchase demand and order | 4 | 7 | 1 | 5 | |
| H Receipt and supplier deviations | 6 | 3 | 3 | 7 | |
| I Supplier invoice and payment | 7 | 3 | 1 | 1 | |
| J Warehouse and stock | 2 | 3 | 1 | 5 | |
...
Most of the 73 gaps come from a few structural decisions or absences, not from single cases.
...
## Missing tests the model already supports

These 19 are the cheapest proof: the behavior exists and only a test is lacking.
A13, A20, B04, C06, D02, D03, E09, E10, F03, F13, G14, H10, H11, H12, I05, J08, L01, L09, O04.
...
| A13 | missing | packages/reality-core/src/reality/services/order_actions.py (line `promised_at`); tests/test_storyline_package.py | A per-line `promised_at` is supported. No service test asserts two commitments of one order with different `due_at`. |
...
| A20 | missing | packages/reality-core/tests/test_shipment_story.py::test_customer_dispatch_records_exact_package_contents_and_fulfillment | Dispatch accepts movements for several commitments, but no test fulfils two orders of one customer in one shipment. |
...
| B04 | missing | services/core.py `release_reservation` + `reserve` | Can only be done as release then reserve again; nothing links the two events and no test covers the move. |
...
| C06 | missing | services/finance/credits.py, tests/test_unified_customer_refund.py (refund flow only) | Unallocated credit plus an evidenced refund could express it; no test pays after an order is cancelled and checks for an owed refund. |
...
| D02 | missing | packages/reality-core/src/reality/services/shipments.py (movements per package carry their own from_location_id) | The model allows it, but no test ships one commitment from two locations in two packages. |
| D03 | missing | packages/reality-core/src/reality/services/shipments.py `_details` (collects a set of commitment_ids per shipment) | Several commitments per package are supported; every dispatch test uses one commitment per shipment. |
...
| E09 | missing | src/reality/db/core.py Document.ship_to_party_id; tests/test_payment_intake.py::test_invoice_core_refuses_another_party_than_the_order | ship_to_party_id exists, but no test uses a recipient different from the billed party, and intake refuses an invoice party that differs from the order. |
| E10 | missing | src/reality/services/artifacts.py, services/file_interpreters.py (CSV/JSON only) | Generic lossless artifact storage exists; there is no XRechnung/ZUGFeRD interpreter and no test storing one. |
...
| F03 | missing | tests/test_return_announcements.py::test_a_movement_refuses_a_foreign_announcement (related only) | A different item cannot fulfil the announcement. The expected result (unexplained return plus announcement still open) is untested. |
...
| F13 | missing | tests/test_credit_notes.py (document_date set, never asserted against period); docs/features/ledger.md (period closing is a non-goal) | No test puts a prior-month invoice and a next-month credit on their LedgerEntry effective_at dates. |
...
| G14 | missing | services/supply_assignments.py::supply_coverage | `protecting_supply` should sum several supplier commitments; no test uses two suppliers for one demand. |
...
| H10 | missing | services/shipments.py shipment_receive (multi-movement) | One package with movements to several commitments is supported; no test does it. |
| H11 | missing | — | Nothing refuses it, but no test or read reports an early receipt. |
| H12 | missing | tests/test_movement_corrections.py::test_replacement_is_atomic_and_becomes_the_only_net_effect | A replacement could add `commitment_id` later; untested. |
...
| I05 | missing | tests/test_multi_position_invoices.py (lines from one order only) | One supplier invoice billing lines of several POs is untested. |
...
| J08 | missing | tests/test_inventory_costing_services.py::test_inventory_transfer_allocation_cannot_invent_owner_stock | This could be a transfer to a location that holds stock, with ownership kept in the spec 242 valuation; but a location has no party link and no test covers stock held at a customer. |
...
| ID | Status | Evidence | Note |
|---|---|---|---|
| L01 | missing | services/core.py create_location (free-form type) | An external location is representable, but no test covers one and no Amazon report interpreter exists. |
...
| L09 | missing | services/core.py `_shopify_interpretation` (accepts price 0) | Zero-price lines are accepted, but no test asserts a free-item line and its commitment. |
...
| O04 | missing | services/core.py lifecycle (is_active toggle only); tests/test_master_data_update_audit.py::test_lifecycle_event_records_before_and_after_and_noop_is_silent | Deactivation doesn't appear to block operations, but no test serves an open commitment for a delisted item. |
