...
The canonical international-v3 profile contains 16 items, 20 customers, 3 suppliers, 2 locations, ten distinct operational cases and 84 days of historical evidence in two comparable 42-day windows. Its bounded costing portfolio contains six complete source-backed contributions plus a visibly incomplete missing-cost trade and a late-cost/customer-return trade. Fixture A derives 40 remaining pcs valued at EUR 420 and 60 billed/fulfilled pcs cost-matched at EUR 630; EUR 1,200 net revenue yields DB1 of EUR 570 and EUR 90 direct plus EUR 24 allocated selling costs yield DB2 of EUR 456 (38%). Five additional exact cases cover healthy DB2 (EUR 125), low positive DB2 (EUR 10), negative DB2 (EUR −30), selling costs explicitly reviewed as zero (DB2 EUR 80) and an allocated-cost-heavy outcome (DB2 EUR 60). All six share one current joint review boundary. These are read-time observations, never stored as new authority. The profile also settles what it bills and buys (feature 204): its sales invoices are paid in full, paid in part or left open, and six purchase orders cover the whole purchase-to-pay chain — not received, partly received, received without an invoice, invoiced and unpaid, invoiced and partly paid, invoiced and settled. Every payment and costing decision goes through the same application services a person uses, under the profile's narrow initialization authority. Every seeded order, invoice and credit note states an authored buyer from that pool: three regulars hold repeat business and the remaining customers hold one or two orders each, so the baseline reads as an order book rather than one customer's ledger. Both windows of a comparison family state the same buyer, and the three purchase orders state the supplier that makes their material. Source-stated values, stable international names and actual ingestion timestamps remain distinct. Separate execution fixtures are fresh isolated tenants, never automatic confirmed actions or destructive resets. Continuous synthetic orders remain outside the reviewed portfolio and keep their default missing-cost semantics.
