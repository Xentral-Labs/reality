...
The canonical international-v8 profile contains 18 items, 20 customers, 3 suppliers, 2 locations, seventeen operational sales cases, nine purchase cases and 84 days of historical evidence in two comparable 42-day windows. In addition to the standard order, return and settlement journeys it includes a price-only customer allowance, an exact invoice posting reversal, two partial invoices on one order, final underdelivery closure, warehouse transfer, separate damage/loss/scrap adjustments, an expired lot, a serial-controlled receipt, an exchange/replacement and a customer prepayment before shipment. Its bounded costing portfolio contains six complete source-backed contributions plus a late-cost/customer-return trade. Fixture A derives 40 remaining pcs valued at EUR 420 and 60 billed/fulfilled pcs cost-matched at EUR 630; EUR 1,200 net revenue yields DB1 of EUR 570 and EUR 90 direct plus EUR 24 allocated selling costs yield DB2 of EUR 456 (38%). Five additional exact cases cover healthy DB2 (EUR 125), low positive DB2 (EUR 10), negative DB2 (EUR −30), selling costs explicitly reviewed as zero (DB2 EUR 80) and an allocated-cost-heavy outcome (DB2 EUR 60). All six share one current joint review boundary. These are read-time observations, never stored as new authority. Sales invoices are paid in full, paid in part or left open. Deterministic Finance examples additionally cover supplier discount, customer and supplier overpayments with available credit, and an explicitly accepted small customer remainder. Every payment, reversal and costing decision goes through the same application services a person uses, under the profile's narrow initialization authority. Source-stated values, stable international names and actual ingestion timestamps remain distinct. Separate execution fixtures are fresh isolated tenants, never automatic confirmed actions or destructive resets. Continuous synthetic orders remain outside the reviewed portfolio and keep their default missing-cost semantics.
