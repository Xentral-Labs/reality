...
## Public model reference (spec 238)

The public Tool Usage explorer has an Analytics model tab next to Data model.
Its static bilingual reference is generated by the existing docs catalog generator
from reporting_catalog, reporting_templates and the validated reporting declaration,
without a database session or tenant observations. It exposes objects, queryable
fields, incoming/outgoing relationships, measures and aggregation restrictions,
service derivation, full technical definitions, templates and execution limits.
Search and business-area filtering are local. Stable `#analytics:<node>` links allow
relationship navigation and guide entry; this reference does not execute questions.

Public explorer terminology (spec238 FR-006): entry names stay English in every
locale, including resources, processes, data-model actions, analytics objects,
fields, relationships, measures and templates. Explanatory prose and controls stay
localized; translated search terms are retained. Runtime catalog labels are unchanged.
