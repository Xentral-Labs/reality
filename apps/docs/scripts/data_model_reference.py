...
RECORDS = [
    ...
    record(
        ...
        "The order from Huber is evidence. Its status is a document lifecycle state, never the delivery, reservation or payment balance.",
        "Hubers Auftrag ist ein Beleg. Sein Status beschreibt den Beleglebenszyklus, niemals Lieferstand, Reservierung oder Zahlungssaldo.",
    ...
    record(
        ...
        {"sku": "LAMP-01", "description": "Lamp", "quantity": "30.0000", "unit": "pcs"},
    ...
    record(
        ...
        "The promise to Huber starts with 30 lamps and an original date. Use revise_commitment for a newly stated date or quantity; original fields remain. Tools also control lifecycle state.",
        "Die Zusage an Huber beginnt mit 30 Lampen und einem ursprünglichen Termin. Einen neu genannten Termin oder eine Menge erfasst revise_commitment; die ursprünglichen Felder bleiben. Tools führen auch den Lebenszyklusstatus.",
    ...
    record(
        ...
        {"due_at": "2026-09-21T12:00:00Z", "note": "Huber agreed a later delivery"},
    ...
    record(
        ...
        "Eight available lamps are allocated to Huber's promise. The shortest evidence path is Reservation → Commitment → DocumentLine. Releasing a reservation changes its status through a tool.",
        "Acht vorhandene Lampen werden Hubers Zusage zugeordnet. Der kürzeste Nachweisweg ist Reservation → Commitment → DocumentLine. Freigeben ändert den Reservierungsstatus über ein Tool.",
    ...
    record(
        ...
        "Eighteen lamps left the warehouse for Huber. Corrections use correct_movement with correction evidence, not an untracked overwrite. Returns have their own meaning; they do not simply reopen a kept promise.",
        "18 Lampen haben das Lager für Huber verlassen. Korrekturen erfolgen mit correct_movement und Korrekturbeleg, ohne unprotokolliertes Überschreiben. Retouren haben eine eigene Bedeutung; sie öffnen eine erfüllte Zusage nicht einfach wieder.",
    ...
    record(
        ...
        "Use supported predicates and their value contracts. Huber's delivery instruction adds context; a changed operational date belongs to a commitment revision. Not every extra ERP field automatically becomes a Fact.",
        "Verwende unterstützte Prädikate und deren Werteformat. Hubers Lieferanweisung ergänzt Kontext; ein geänderter operativer Termin gehört in eine Zusagenrevision. Nicht jedes zusätzliche ERP-Feld wird automatisch ein Fact.",
...
for key, name, en, de, example, note_en, note_de, derived_en, derived_de, related in [
    (
        ...
        {"name": "Huber", "type": "customer", "default_currency": "EUR"},
    ...
    (
        ...
        {"role": "customer"},
        "Huber's customer role points to the existing Party. Adding a supplier role does not create a second address book.",
        "Hubers Kundenrolle verweist auf den vorhandenen Party-Datensatz. Eine zusätzliche Lieferantenrolle erzeugt kein zweites Adressbuch.",
    ...
    (
        ...
        {"sku": "LAMP-01", "name": "Lamp", "unit": "pcs", "item_type": "stocked"},
    ...
    (
        ...
        {"priority": 100},
        "The assignment connects Huber to a price list with priority and optional validity. It does not copy the list's prices.",
        "Die Zuordnung verbindet Huber mit einer Preisliste samt Priorität und optionaler Gültigkeit. Sie kopiert nicht deren Preise.",
    ...
    (
        ...
        "A shipment groups the transport to Huber. Packages hold tracking details; transport events report progress. Goods are linked through package movements.",
        "Eine Sendung bündelt den Transport zu Huber. Packstücke halten Trackingangaben; Transportereignisse melden den Verlauf. Die Ware ist über Packstückbewegungen verknüpft.",
