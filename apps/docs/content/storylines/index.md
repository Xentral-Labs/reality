...
current story's Sandbox; the protocol keeps recording. **Free Play** is a separate tile in the
Storyline library, with no sidebar entry of its own. Choose your current company, another accessible
company, or create a persistent Sandbox with sample data. Existing companies use their actual data;
changes require confirmation. It needs no Storyline or prescribed steps. Recorded reply evidence is
available in Storyline and Free Play Sandboxes; elsewhere missing evidence is stated explicitly.
Autoplay runs the steps on a timer for a presentation and stops at any click.
