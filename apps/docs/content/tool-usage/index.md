...
`{"code": "...", "message": "...", "tool": "..."}`. The code is stable and says why Reality
refused: `not_found` (the record the arguments name does not exist in this tenant),
`invalid_operation` (the request is not valid for this record or these arguments),
`conflict` (the request was valid in shape but based on stale state), `needs_review` (an
interpreter declined because the business meaning is ambiguous), `reality_error` (any other
business-readable refusal). The message is the sentence a person reads; it is for people and
must not be parsed. An agent that has to decide whether to retry, ask, or stop reads the code
and shows the message. Failures that are not refusals, such as a missing scope or an unknown
tool, keep their plain text and no code; they are not business outcomes.
