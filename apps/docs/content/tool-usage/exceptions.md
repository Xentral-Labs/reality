...
Deterministically derived conditions that need attention. Each one names who owns it, what clears
it, and which agent tools list and explain it.

> Automatically generated from `operational_exception_catalog.yaml`. Do not edit this page by hand.
...
| [`shipped_not_billed`](#exception-shipped_not_billed)                                     | Shipped and not billed                   | Finance                    | `high`   | Billing, with order fulfilment when the delivery is in doubt                                |
| [`billed_not_received`](#exception-billed_not_received)                                   | Billed and not received                  | Finance                    | `high`   | Accounts payable, with purchasing when the goods are missing                                |
| [`invoice_price_differs`](#exception-invoice_price_differs)                               | Invoice price differs from the agreement | Finance                    | `high`   | Accounts payable for a supplier invoice, billing for a customer invoice                     |
| [`sold_below_purchase_price`](#exception-sold_below_purchase_price)                       | Sold below the purchase price            | Finance                    | `high`   | Sales management, with purchasing when the purchase price is the stale figure               |
| [`returned_not_credited`](#exception-returned_not_credited)                               | Returned and not credited                | Finance                    | `high`   | Customer service, with billing when the credit note is the missing step                     |
| [`credited_not_returned`](#exception-credited_not_returned)                               | Credited and not returned                | Finance                    | `high`   | Customer service, with credit control when the money is already gone                        |
| [`supplier_return_not_credited`](#exception-supplier_return_not_credited)                 | Returned to supplier and not credited    | Finance                    | `high`   | Purchasing, with accounts payable when the money has already gone                           |
| [`supplier_credit_not_returned`](#exception-supplier_credit_not_returned)                 | Supplier credited more than went back    | Finance                    | `high`   | Purchasing, with accounts payable at period end                                             |
| [`return_unresolved`](#exception-return_unresolved)                                       | Return not dealt with                    | Orders & fulfilment        | `normal` | Warehouse control, with customer service when the decision is theirs                        |
| [`receipt_unbilled`](#exception-receipt_unbilled)                                         | Receipt not invoiced                     | Finance                    | `high`   | Purchasing, with accounts payable at period end                                             |
| [`units_not_comparable`](#exception-units_not_comparable)                                 | Units not comparable                     | Cross-functional           | `normal` | Whoever maintains item master data, with purchasing                                         |
| [`reservation_exceeds_stock`](#exception-reservation_exceeds_stock)                       | Reservation exceeds stock                | Orders & fulfilment        | `high`   | Warehouse control, with purchasing when stock has to be replaced                            |
| [`silent_source`](#exception-silent_source)                                               | Silent source                            | Documents, sources & facts | `high`   | Integration operations, with the owner of the source system                                 |
| [`source_interpretation_failure`](#exception-source_interpretation_failure)               | Source interpretation failure            | Documents, sources & facts | `high`   | Integration operations, with the owner of the source system                                 |
| [`unexplained_movement`](#exception-unexplained_movement)                                 | Unexplained movement                     | Warehouse & logistics      | `normal` | Warehouse control                                                                           |
| [`sales_invoice_unposted`](#exception-sales_invoice_unposted)                             | Sales invoice not booked                 | Finance                    | `high`   | Billing, with accounts receivable at period end                                             |
| [`supplier_invoice_unposted`](#exception-supplier_invoice_unposted)                       | Supplier invoice not booked              | Finance                    | `high`   | Accounts payable                                                                            |
| [`credit_note_unposted`](#exception-credit_note_unposted)                                 | Credit note not booked                   | Finance                    | `high`   | Accounts receivable                                                                         |
| [`credit_note_unsettled`](#exception-credit_note_unsettled)                               | Credit note not given back               | Finance                    | `high`   | Accounts receivable, with treasury when the money has to leave                              |
| [`supplier_credit_unposted`](#exception-supplier_credit_unposted)                         | Supplier credit not booked               | Finance                    | `high`   | Accounts payable                                                                            |
| [`supplier_credit_unclaimed`](#exception-supplier_credit_unclaimed)                       | Supplier credit not claimed              | Finance                    | `high`   | Accounts payable, with purchasing when the supplier has to be chased                        |
| [`overdue_receivable`](#exception-overdue_receivable)                                     | Overdue receivable                       | Finance                    | `high`   | Accounts receivable                                                                         |
| [`credit_limit_exceeded`](#exception-credit_limit_exceeded)                               | Credit limit exceeded                    | Finance                    | `high`   | Credit control, with sales when the limit itself is the question                            |
| [`overdue_payable`](#exception-overdue_payable)                                           | Overdue payable                          | Finance                    | `high`   | Accounts payable                                                                            |
| [`purchase_discount_available`](#exception-purchase_discount_available)                   | Early payment discount still available   | Finance                    | `normal` | Accounts payable, with whoever schedules the payment run                                    |
| [`duplicate_supplier_invoice`](#exception-duplicate_supplier_invoice)                     | Duplicate supplier invoice               | Finance                    | `high`   | Accounts payable                                                                            |
| [`unmatched_financial_event`](#exception-unmatched_financial_event)                       | Unmatched financial event                | Finance                    | `high`   | Accounts receivable or accounts payable                                                     |
| [`announced_return_not_arrived`](#exception-announced_return_not_arrived)                 | Announced return has not arrived         | Orders & fulfilment        | `normal` | Customer service, with the receiving desk once the parcel is expected                       |
...
- **Clears through:** Shipping the outstanding quantity, or cancelling the commitment.
- **Severity:** `high`
- **Record type:** `commitment`
- **Authority:** `068/FR-001`
- **Evidence:**
...
- **Owner:** Order fulfilment or warehouse operations
- **Clears through:** Reserving the remaining quantity, shipping it, or cancelling the commitment.
- **Severity:** `high`
- **Record type:** `commitment`
- **Authority:** `020/FR-004`
- **Evidence:** `tests/operational_exceptions/test_derivation.py::test_outgoing_commitment_at_risk`
...
- **Clears through:** Shipping the outstanding quantity, cancelling the order, or agreeing a date
  with the customer.
- **Severity:** `high`
- **Record type:** `commitment`
...
- **Owner:** Purchasing or inbound operations
- **Clears through:** Receiving the outstanding quantity, or cancelling the commitment.
- **Severity:** `high`
- **Record type:** `commitment`
- **Authority:** `020/FR-005`
- **Evidence:**
...
- **Owner:** Accounts payable, with purchasing when the goods are missing
- **Clears through:** Receiving the outstanding quantity, or correcting the invoice.
- **Severity:** `high`
- **Record type:** `document_line`
...
- **Owner:** Accounts payable for a supplier invoice, billing for a customer invoice
- **Clears through:** Correcting the invoice line, or agreeing the new price on the order line.
- **Severity:** `high`
- **Record type:** `document_line`
...
- **Owner:** Sales management, with purchasing when the purchase price is the stale figure
- **Clears through:** Agreeing a price at or above the purchase price, or correcting the purchase
...
- **Owner:** Purchasing, with accounts payable when the money has already gone
- **Clears through:** The supplier's credit note naming the order line, or the goods coming back
  again.
- **Severity:** `high`
- **Record type:** `document_line`
...
- **Owner:** Purchasing, with accounts payable at period end
- **Clears through:** The remaining goods going back, or the supplier correcting its credit note.
- **Severity:** `high`
- **Record type:** `document_line`
...
- **Owner:** Warehouse control, with customer service when the decision is theirs
- **Clears through:** Restocking, writing off, or sending the goods back to the supplier, saying
...
- **Owner:** Purchasing, with accounts payable at period end
- **Clears through:** The supplier invoice arriving and an invoice line naming the order line.
- **Severity:** `high`
- **Record type:** `document_line`
...
- **Owner:** Warehouse control, with purchasing when stock has to be replaced
- **Clears through:** Receiving stock, or releasing reservations until they fit what is there.
- **Severity:** `high`
- **Record type:** `item`
...
- **Owner:** Integration operations, with the owner of the source system
- **Clears through:** A record arriving from that source.
- **Severity:** `high`
- **Record type:** `source_capability`
...
- **Owner:** Integration operations, with the owner of the source system
- **Clears through:** A successful retry once the cause of the failure is removed.
- **Severity:** `high`
- **Record type:** `import_job`
- **Authority:** `020/FR-006`
- **Evidence:**
...
- **Clears through:** Nothing. The movement stays part of history; only a correction changes what
  follows from it.
- **Severity:** `normal`
- **Record type:** `movement`
...
- **Owner:** Billing, with accounts receivable at period end
- **Clears through:** Booking the invoice, or correcting it if it should never have been raised.
- **Severity:** `high`
...
- **Clears through:** Booking the invoice, or rejecting it if the supplier should never have sent
  it.
- **Severity:** `high`
- **Record type:** `document`
...
- **Clears through:** Booking the credit note, or cancelling it if it should never have been raised.
- **Severity:** `high`
- **Record type:** `document`
...
- **Owner:** Accounts receivable, with treasury when the money has to leave
- **Clears through:** Netting the credit against an open invoice, or refunding the customer.
- **Severity:** `high`
- **Record type:** `document`
...
- **Owner:** Accounts payable
- **Clears through:** Booking the supplier credit note, or rejecting it if the supplier should never
  have sent it.
- **Severity:** `high`
- **Record type:** `document`
...
- **Owner:** Accounts payable, with purchasing when the supplier has to be chased
- **Clears through:** Netting the credit against an open supplier invoice, or having the supplier
  refund it.
- **Severity:** `high`
- **Record type:** `document`
...
- **Owner:** Accounts receivable
- **Clears through:** Settling the outstanding amount, or reversing the invoice.
- **Severity:** `high`
- **Record type:** `document`
- **Authority:** `069/FR-001`
- **Evidence:** `tests/operational_exceptions/test_derivation.py::test_overdue_receivable`
...
- **Owner:** Credit control, with sales when the limit itself is the question
- **Clears through:** Settling enough of the open invoices, or agreeing and recording a higher
  limit.
- **Severity:** `high`
- **Record type:** `party`
...
- **Owner:** Accounts payable
- **Clears through:** Paying the outstanding amount, or reversing the invoice.
- **Severity:** `high`
- **Record type:** `document`
- **Authority:** `074/FR-001`
- **Evidence:** `tests/operational_exceptions/test_derivation.py::test_overdue_payable`
...
- **Owner:** Accounts payable, with whoever schedules the payment run
- **Clears through:** Paying the invoice, on its own or in a payment run. It also stops appearing
...
- **Owner:** Accounts payable
- **Clears through:** Reversing whichever posting was made in error, or confirming the numbers
  differ.
- **Severity:** `high`
- **Record type:** `document`
...
- **Clears through:** Allocating the remainder to the invoices it pays.
- **Severity:** `high`
- **Record type:** `ledger_entry`
...
- **Owner:** Customer service, with the receiving desk once the parcel is expected
- **Clears through:** The goods arriving against the announcement, or the customer withdrawing it.
- **Severity:** `normal`
...
- **Owner:** Whoever raised the hold, named in the entry, with the reason code saying which team
  that is
- **Clears through:** Lifting the hold, or doing the thing it was raised for and then lifting it.
- **Severity:** `normal`
- **Record type:** `commitment_hold`
...
- **Evidence:** `tests/operational_exceptions/test_derivation.py::test_stock_expired`
