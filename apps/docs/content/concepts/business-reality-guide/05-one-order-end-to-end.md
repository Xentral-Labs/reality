...
## 1. Your understanding check: is Huber's order complete?

All 30 lamps are recorded as shipped from Acme to Huber. Invoice `INV-1001` states EUR 1,470, with
EUR 500 payment allocated and EUR 100 credit applied. There are no further movements, reservations
or financial entries in this base sequence.
...
record `SO-1001`. Acme's **Commitment** to Huber holds the delivery promise; each **Reservation**
allocates stock to it and each fulfilling **Movement** records a shipment.
...
| Huber orders 30                              | Order, line and customer Commitment                    | Thirty to deliver                                           |
| Allocate the existing eight                  | Reservation                                            | Eight reserved, 22 unreserved                               |
| Order 22 from LightWorks                     | PO-2001 and supplier Commitment                        | Twenty-two expected                                         |
...
| Ship 18 and twelve to Huber                  | Two shipment Movements against the customer Commitment | Thirty fulfilled, zero open; matching Reservations consumed |
...
> Delivery for Huber's order SO-1001 is fully recorded as shipped: 18 and twelve lamps against the
> promise of 30. No active reservation remains for this order. The linked invoice INV-1001 has EUR
> 870 outstanding: EUR 1,470 invoiced less EUR 500 allocated payment and EUR 100 applied credit. I
> cannot establish whether that remainder is overdue without checking the due date. Next I would
> check the due date and further incoming payments. I have not changed any records.

In a real answer, delivery and reservation claims must lead to Huber's Commitment and its records.
The open amount must lead to the invoice posting, payment, SettlementAllocation and credit. Where
available, provenance can be followed to the original input. Readable example numbers do not replace
those technical relationships.
