...
INFRA = {
 "tenant","app_user","user_session","email_verification_code","tenant_membership",
 "access_application","access_admission_counter","security_audit_event","company_invitation",
 "invitation_delivery","chat_session","chat_message","ai_settings","secret","secret_audit_event",
 "mcp_access_token","playground_run","playground_step","storyline_trace_entry","storyline_package",
 "analytics_report","analytics_report_draft","business_event","projection_row",
 "projection_checkpoint","action","reality_gap","reality_gap_entry","interpretation_rule",
 "rule_interpretation_outcome","import_job","source_system","source_capability","source_stream",
 "source_artifact","interpretation_outcome","interpretation_record_reference","scheduled_job",
 "scheduled_job_run","demo_data_connection","ordinary_company_creation","opening_scope",
 "opening_item_detail","source_classification_mapping_revision",
}
...
VOCABULARY = {
    ("document", "type"): {
        "sales_order": 30, "purchase_order": 14,
        "sales_invoice": 44, "supplier_invoice": 35,
        "credit_note": 33, "supplier_credit_note": 13,
    },
    ("commitment", "type"): {"customer_delivery": 8, "supplier_delivery": 6},
    ("movement", "type"): {
        "opening_stock": 22, "receipt": 89, "shipment": 59, "return": 33,
        "supplier_return": 19, "transfer": 3, "adjustment": 7, "correction": 13,
    },
}
...
def main():
    ...
    aux = {v[k]["table"] for v in nodes.values()
           for k in ("correction_table", "revision_table") if v.get(k)}
    ...
    print("=" * 72)
    print(f"Geschäftstabellen: {len(biz)} | als Knoten: {len(biz & node_tables)} | "
          f"als Korrektur-/Revisionstabelle: {len(biz & aux)}")
    open_edges = []
    for t in meta.tables.values():
        if t.name in INFRA: continue
        for col in t.columns:
            if col.name == "tenant_id": continue
            for fk in col.foreign_keys:
                tgt = fk.column.table.name
                if tgt in INFRA or tgt == "source_record": continue
    ...
    print(f"Fremdschlüssel ohne Kante: {len(open_edges)} "
    ...
    for (table, column), counts in VOCABULARY.items():
        ...
        for name, n in nodes.items():
            if n.get("table") != table: continue
            w = (n.get("where") or {}).get(column)
            if w is None: covered |= {"<alle>"}
            elif isinstance(w, list): covered |= set(w)
            else: covered.add(w)
    ...
    print(f"   data_model.yaml kennt {len(dm_tables)} Tabellen, das Schema hat {len(live)}")
