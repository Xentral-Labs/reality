...
Date: 2026-09-17. Isolated worktree initially based on `f21aecfd`, rebased onto `377af570` before the final checks. The newly merged graph tools are included.
...
- `gmake web-build spec-check lint docs-catalog-check`: **PASS**. Includes 237 frontend tests, TypeScript/Vite production build, localization checks (1,904 statically audited keys in all four locales), specification policy, Ruff and regenerated documentation consistency. Vite retains its existing bundle-size advisory.
...
- Exact sorted JSON comparison of all **152** MCP public definitions against the current-main baseline: **identical** (names, descriptions, input schemas, access modes and other public metadata). SHA-256: `69aed7648fb141b823d051e88af3245f87efe724a1e9af3b2bbd33b9906a9db1`.
...
- Coverage checks represent all 93 commands, 13 projections, 17 workspace views, workspace actions, discovery entries and 152 MCP definitions across 154 presentation capabilities. Explicit read aliases merge; distinct movement forms remain separate.
