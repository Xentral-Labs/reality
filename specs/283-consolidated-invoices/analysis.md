...
| A4 | LOW | `billable_positions` built on `_order_line_billing` per line would issue queries per line. | T011 proves the statement count does not grow with fully billed history (SQL prefilter). Open candidates are still read per line through the shared rules, bounded by the 200 limit; duplicating those rules in SQL would create a second truth. |
